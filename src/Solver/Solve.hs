...
ubica num top row column (x:xs)  hidato mask | top == num && placed = (True, hidato)
                                             ...
                                             | already_ubicated num x && placed = ubica new_num top placed_row placed_column xs hidato mask
                                             | already_ubicated num x = (False , hidato) 
...
valid_place :: (Num a , Ord a) => a -> a -> a -> a -> [a] -> [[a]] -> [[Bool]] -> (Bool, [[a]])
valid_place num top row column  ubicated hidato mask | not_valid_in_mask row column mask = (False, hidato)
                                                     | not_valid_in_hidato row column hidato = (False, hidato)
